...
// Definimos la interfaz para evitar el error 'any'
...
export default function VotarPage() {
  const supabase = useMemo(() => createClient(), []); // Usamos useMemo para que la instancia sea estable
  ...
  useEffect(() => {
    async function cargar() {
      ...
      const cuines: Restaurant[] = planData?.cuines_seleccionades || []
    ...
    cargar()
  }, [params.id, supabase]) // Dependencias añadidas para corregir el warning
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-sm">
        ...
        <div
          className="border border-border rounded-2xl overflow-hidden mb-6 transition-all duration-300 bg-card"
          style={{
        ...
        >
          <div className="relative w-full h-48">
            {restaurante.foto ? (
              <Image
                ...
                unoptimized // Añadido para URLs dinámicas de API si dan problemas
              />
            ) : (
          ...
          <div className="p-6">
            ...
            {restaurante.id && (
              <a href={`https://www.google.com/maps/search/?api=1&query=Google&query_place_id=${restaurante.id}`}
