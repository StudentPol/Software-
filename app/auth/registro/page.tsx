...
function RegistroForm() {
  ...
  async function handleRegistro() {
    if (!nombre.trim()) { setError('El nom és obligatori'); return }
    ...
    <main className="min-h-screen bg-background flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground">Crea el teu compte</p>
        </div>
        ...
        <div className="flex flex-col gap-5">
          <div>
            <label className="text-sm text-muted-foreground block mb-2">Nom</label>
            <input type="text" placeholder="El teu nom" value={nombre} onChange={e => setNombre(e.target.value)}
          ...
          <div>
            <label className="text-sm text-muted-foreground block mb-2">Contrasenya</label>
          ...
          <button onClick={handleRegistro} disabled={cargando}
            ...
            {cargando ? 'Creant compte...' : 'Crear compte →'}
          </button>
          <p className="text-center text-sm text-muted-foreground">
            Ja tens compte?{' '}
            <a href={`/auth/login${searchParams.get('redirect') ? `?redirect=${searchParams.get('redirect')}` : ''}`}
              className="text-foreground underline underline-offset-4">Inicia sessió</a>
          </p>
        </div>
      </div>
    </main>
