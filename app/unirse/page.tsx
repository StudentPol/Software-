...
function UnirseForm() {
  ...
  async function handleUnirse() {
    if (!codigo.trim()) { setError('Introduce el código del plan'); return }
    setCargando(true)
    setError('')
    ...
    const { data: plan, error: planError } = await supabase
      .from('planes').select('*').eq('codigo', codigo.toUpperCase()).single()
    if (planError || !plan) { setError('Código incorrecto, compruébalo'); setCargando(false); return }
    const { data: miembroExiste } = await supabase
      .from('miembros').select('id').eq('plan_id', plan.id).eq('user_id', user.id).single()
    if (!miembroExiste) {
      await supabase.from('miembros').insert({ plan_id: plan.id, user_id: user.id })
    }
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="max-w-3xl mx-auto px-8 pb-10">
        <div className="w-full max-w-md mx-auto">
          <a href="/" className="text-sm text-muted-foreground hover:text-foreground mb-6 inline-block">← Volver</a>
          <h2 className="text-2xl font-medium mb-2">Unirme a un plan</h2>
          <p className="text-muted-foreground text-sm mb-8">Introduce el código que te ha compartido tu amigo</p>
          <div className="flex flex-col gap-5">
            <div>
              <label className="text-sm text-muted-foreground block mb-2">Código del plan</label>
              <input type="text" placeholder="Ej: PLAN-ABC123" value={codigo} onChange={e => setCodigo(e.target.value)}
            ...
            <button onClick={handleUnirse} disabled={cargando}
...
export default function UnirseASala() {
  return <Suspense><UnirseForm /></Suspense>
}